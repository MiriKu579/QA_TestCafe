...
// for debug use .only or .skip
    ...
    test.only('Shopping multiple item', async t=>{
        ...
        await t.expect(ShoppingPage.itemDescription.innerText).contains(ITEMS.BABYSHIRT.ITEM_DESCRIPTION)
        await t.expect(ShoppingPage.item.innerText).contains(ITEMS.BAGPACK.ITEM_NAME)
        await t.expect(ShoppingPage.itemDescription.innerText).contains(ITEMS.BAGPACK.ITEM_DESCRIPTION)
        await t.expect(ShoppingPage.item.innerText).contains(ITEMS.TSHIRT_RED.ITEM_NAME)
        await t.expect(ShoppingPage.itemDescription.innerText).contains(ITEMS.TSHIRT_RED.ITEM_DESCRIPTION)
        await t.expect(ShoppingPage.item.innerText).contains(ITEMS.JACKET.ITEM_NAME)
        await t.expect(ShoppingPage.itemDescription.innerText).contains(ITEMS.JACKET.ITEM_DESCRIPTION)
        await t.expect(ShoppingPage.item.innerText).contains(ITEMS.BIKELIGHT.ITEM_NAME)
        await t.expect(ShoppingPage.itemDescription.innerText).contains(ITEMS.BIKELIGHT.ITEM_DESCRIPTION)
    })